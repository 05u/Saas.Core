...
{
    public class RedisStackExchangeService : IRedisStackExchangeService
    {
        private readonly ConnectionMultiplexer _conn;
        public string CustomKey;
        public string SysCustomKey = string.Empty;
        ...
        public RedisStackExchangeService(IConfiguration configuration,
             //,
...

        {
            ...
            _conn = ConnectionMultiplexer.Connect(Configuration.GetSection("Redis:ConnectString").Value);
            ...
            _easyCachingProviderFactory = cachingProviderFactory;
            _cachingProvider = _easyCachingProviderFactory.GetCachingProvider("redis1");
        ...
        ///// <param name="labelCodeType"></param>
        ///// <param name="count"></param>
        ///// <returns></returns>
        //public List<long> GetSerialNumber(RedisUserLockType lockType, RedisLabelCodeType labelCodeType, int count = 1) 
        //{

        //  var key = (lockType == RedisUserLockType.Company
        //         ? _currentUserService.GetCompanyId()
        //         : _currentUserService.GetUserId());
        //   return GetSerialNumber(key, count);
        //}
        ...
        /// <param name="ts"></param>
        /// <returns></returns>
        public async Task<List<int>> CacheCountCheck(string cacheKey, TimeSpan? ts, int count = 1)
        {
            ts ??= new TimeSpan(24, 0, 0);
            var cache = await _cachingProvider.GetAsync<int>(cacheKey);
            var realCount = 0;//可能0次
            if (cache.HasValue)
            {
                realCount = cache.Value;
            }
            var cacheValue = realCount;
            realCount += count;
            await _cachingProvider.UpdateAsync(cacheKey, realCount, ts.Value);
            return Enumerable.Range(cacheValue, count).ToList();
        ...
        /// 获取流水号
        /// </summary>
        public List<long> GetStaticSerialNumber(string key, int count = 1)
        {
            ...
            {
                list.Add(db.StringIncrement(key, 1));
            }
        ...
        /// 获取流水号
        /// </summary>
        public List<long> GetSerialNumber(string key, int count = 1)
        {
            List<long> list = new List<long>();
            var db = _conn.GetDatabase();
            if (!db.KeyExists(key))
            {
                db.StringSet(key, 0, ToZero());
            }
        ...
        private TimeSpan ToZero()
        {
            DateTime nowDate = DateTime.Now;
            TimeSpan now = TimeSpan.FromTicks(nowDate.Ticks);
            TimeSpan zero = TimeSpan.FromTicks(new DateTime(nowDate.Year, nowDate.Month, nowDate.Day, 23, 59, 59).Ticks);
        ...
        /// <param name="val">可以为负</param>
        /// <returns>减少后的值</returns>
        public async Task<double> StringDecrementAsync(string key, long val = 1)
        {
            key = AddSysCustomKey(key);
        ...
        {
            var prefixKey = CustomKey ?? SysCustomKey;
